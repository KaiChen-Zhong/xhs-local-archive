...
test("native host classifyAll prefill assigns controlled paths before slow AI refinement", async () => {
  await handleMessage({ type: "saveSettings", settings: { ai: {} }, clearAiKey: true });
  const existing = await handleMessage({ type: "listNotes" });
  await handleMessage({ type: "deleteLocal", noteIds: existing.notes.map((note) => note.noteId) });
  const notes = [
    {
      noteId: `prefill-ai-${Date.now()}`,
      title: "Opus 和 Claude Code 的 AI 工具更新",
      url: "https://www.xiaohongshu.com/explore/prefill-ai",
      ai: { categoryPath: ["未分类", "待细分"], category: "未分类", subcategory: "待细分", source: "local" }
    },
    {
      noteId: `prefill-finance-${Date.now()}`,
      title: "纳指 ETF 定投和美股回撤怎么看",
      url: "https://www.xiaohongshu.com/explore/prefill-finance",
      ai: { categoryPath: ["未分类", "待细分"], category: "未分类", subcategory: "待细分", source: "local" }
    }
  ];
  assert.equal((await handleMessage({ type: "upsertNotes", notes })).ok, true);
  const classified = await handleMessage({ type: "classifyAll", forceUnclassified: true, prefillOnly: true });
  assert.equal(classified.ok, true);
  assert.equal(classified.mode, "prefill");
  assert.equal(classified.succeeded, notes.length);
  const listed = await handleMessage({ type: "listNotes" });
  assert.deepEqual(listed.notes.find((note) => note.noteId === notes[0].noteId).ai.categoryPath, ["科技", "AI工具"]);
  assert.deepEqual(listed.notes.find((note) => note.noteId === notes[1].noteId).ai.categoryPath, ["金融", "股票基金"]);
});
