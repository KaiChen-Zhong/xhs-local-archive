...
test("extension keeps conservative scan and long archive safeguards", () => {
  ...
  assert.match(serviceWorker, /AUTO_ARCHIVE_FAILURE_LIMIT\s*=\s*80/);
  ...
  assert.match(sidePanelHtml, /id="jobStatus"/);
  assert.match(sidePanelHtml, /id="retryBackgroundJobs"/);
  assert.match(sidePanelHtml, /id="captureHealth"/);
  assert.match(sidePanelHtml, /导出全部数据备份/);
  assert.match(sidePanelHtml, /<section class="diagnostics" hidden>/);
  assert.match(sidePanel, /describeResponse/);
  assert.match(sidePanel, /renderBackgroundJobStatus/);
  assert.match(sidePanel, /renderCaptureHealth/);
  assert.match(sidePanel, /分类待审/);
  ...
  assert.match(sidePanel, /全部数据备份已导出/);
