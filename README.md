...
- 默认单次扫描上限为 100000 条，适配万级收藏夹；状态栏会显示页面宣称总数、已发现数、覆盖比例、缺标题数、缺封面数和实际滚动容器。
...
- 扫描默认值：自适应滚动步长、1.2 秒等待、100000 条卡片上限；未达到页面宣称总数时不会自动按“完成”停止，会继续重试直到采满、手动停止、风险拦截或达到最大时长。
- 批量分类默认处理全部已采集且仍需分类或 AI 异常的条目；Native Host 默认 5 并发，返回汇总并刷新完整列表。
