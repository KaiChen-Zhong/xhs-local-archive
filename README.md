...
一个 Chrome/Edge MV3 浏览器扩展，配套 Native Messaging 本地宿主，用于把小红书收藏/点赞页中已经可见的卡片整理成本地分类库。当前实现只使用卡片标题和封面做 AI 分类，不打开帖子、不读取正文、不采集评论。
...
- 不打开帖子，不做详情页 hydration，不读取正文，不采集评论。
