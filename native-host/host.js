...
async function handleMessage(message) {
  ...
  if (type === "classifyAll") {
    ...
    const prefilled = prefillLocalClassifications(db, notes);
    if (prefilled) saveDb(db);
    if (message.prefillOnly) {
      logEvent(db, "info", "classify_all_prefill", { count: notes.length, ok: prefilled, limited: Number.isFinite(limit) });
      saveDb(db);
      return {
        ok: true,
        mode: "prefill",
        ...
        succeeded: prefilled,
        failed: Math.max(0, notes.length - prefilled),
...
function needsClassification(note) {
  ...
  if (ai.source === "local_prefill") return true;
...
function mergeAiClassification(existingAi, incomingAi) {
  ...
  const existingUnclassified = isUnclassifiedAi(existingAi);
  const incomingUnclassified = isUnclassifiedAi(incomingAi);
  ...
  if (!existingUnclassified && incomingUnclassified) return { ...merged, categoryPath: existingAi.categoryPath, category: existingAi.category, subcategory: existingAi.subcategory };
...
function inferTaxonomy(note) {
  ...
  const rules = [
    ...
    [/美妆|妆容|口红|粉底|护肤|香水|发型|发色/, ["美妆", "护肤彩妆"]],
    [/旅行|旅游|攻略|路线|酒店|民宿|机票|citywalk|露营/, ["旅行", "攻略目的地"]],
    [/家居|装修|收纳|软装|租房|房间|卧室|客厅/, ["家居", "装修收纳"]],
    [/健身|瑜伽|跑步|普拉提|减脂|运动|训练|臀桥|划船|保加利亚蹲|篮球|台球|斯诺克|弹跳/, ["健康", "运动健身"]],
    [/心脏|急症|膝盖|补剂|全麻|前额叶|拖延|心理|身心|不舒服|疼/, ["健康", "身心护理"]],
    [/学习|读书|考研|英语|笔记|效率|自律|课程|科研|博导|面经|校招|习惯|知识库/, ["学习", "知识成长"]],
    [/\bai\b|大模型|agent|rag|prompt|提示词|ai工具|openai|claude|deepseek|mcp|llm|anthropic|opus|sonnet|mythos|transformer|codex|vibe\s*coding|cursor|copilot|gpt|gemini|模型|ai产品|人工智能|机器学习|神经网络|智谱|minimax|中转站|skill|harness|scrapling|数字画像|科研idea|token/, ["科技", "AI工具"]],
    [/数码|手机|电脑|软件|app|相机|键盘|耳机|域名|开源|星标|github|系统|模拟器|浏览器|服务器|数据库|代码|编程|程序员|开发|产品经理|互联网|chrome|视频平台|部署|网站|工具/, ["科技", "数码工具"]],
    [/股票|股市|熔断|三星|海力士|美股|港股|a股|基金|etf|券商|开户|做空|财报|半导体|牛市|熊市|量价|当冲|指数|估值|科技股|芯片|英伟达|nvidia|纳指|散户|趋势票|回撤|定投/, ["金融", "股票基金"]],
    [/宏观|美联储|降息|加息|美元|汇率|通胀|经济|财富风口|财经|理财|投资|资本|配置机会|对冲|黑天鹅|财富方程式|商业|回报率/, ["金融", "宏观财经"]],
    [/求职|简历|面试|职场|工作|办公|副业|运营|创业|自媒体|打工人|商业先行者/, ["职场", "求职办公"]],
    [/设计|审美|灵感|排版|配色|海报|字体/, ["审美", "设计灵感"]],
    [/拍照|摄影|修图|滤镜|构图|相机/, ["摄影", "拍照修图"]],
    [/电影|电视剧|综艺|音乐|演唱会|明星|追剧|美剧|五月天|周深|女高音|戏|剧本|情书|阿嬷|南枝|潮汕话/, ["娱乐", "影视音乐"]],
    ...
    [/地铁|公交|报站|城市|深圳|北京|上海|广州|厦门|香港|进藏|自驾|路线/, ["旅行", "攻略目的地"]],
...
async function classifyAllBatch(db, notes, candidates, options = {}) {
  ...
  const batchSize = Math.max(1, Math.min(Number(options.batchSize) || CLASSIFY_BATCH_SIZE || 12, 50));
...
function prefillLocalClassifications(db, notes) {
  let changed = 0;
  for (const note of notes || []) {
    if (!note || !note.noteId || (!needsClassification(note) && !isUnclassifiedPath(note))) continue;
    const governed = governClassification(db, finalFallbackClassification(note, "local_prefill"), { source: "local_prefill", noteId: note.noteId });
    note.ai = { ...(note.ai || {}), ...governed, aiPipeline: { mode: "local_prefill" } };
    clearClassificationIncomplete(note);
    if (isUnclassifiedPath(note)) markClassificationIncomplete(note);
    note.updatedAt = new Date().toISOString();
    db.notes[note.noteId] = note;
  ...
  if (changed) logEvent(db, "info", "classify_all_local_prefill", { count: changed });
  return changed;
}
